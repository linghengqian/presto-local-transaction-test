...
public class PrestoTest {
    ...
    void assertLocalTransactions() throws SQLException {
        String baseJdbcUrl = "jdbc:presto://localhost:" + container.getMappedPort(8080) + "/iceberg";
        DataSource logicDataSource = createDataSource(baseJdbcUrl);
        this.assertRollbackWithTransactions(logicDataSource);
    }

    private DataSource createDataSource(String baseJdbcUrl) throws SQLException {
        ...
        try (Connection con = DriverManager.getConnection(baseJdbcUrl, "test", null)) {
            con.createStatement().execute("CREATE SCHEMA iceberg.demo_ds_0");
        }
        try (Connection con = DriverManager.getConnection(baseJdbcUrl + "/demo_ds_0", "test", null)) {
            ...
            con.createStatement().execute("truncate table t_order");
        }
